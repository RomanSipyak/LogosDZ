...
public class Main {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			System.out.println("Enter 3 to sort by age desc");
			switch (sc.next()) {
			case "1":
...
//		return persons;
//		return list.stream()
//		.sorted((o1, o2)->o2.getName().compareTo(o1.getName())).collect(toList());
		return list.stream()
				.sorted(new PersonComparatorByNameDesc()).collect(toList());
	}
	
	static List<Person> sortByAgeAsc(){
		return list.stream()
		.sorted(comparingInt(Person::getAge)).collect(toList());
	}

	static Stream<Person> sortByAgeDesc(){
		return list.stream()
		.sorted(comparingInt(Person::getAge).reversed());//можна і так
	}
