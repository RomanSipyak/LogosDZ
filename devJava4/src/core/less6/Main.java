...
public class Main {
	...
	public static void main(String[] args) {
		...
		System.out.println(month.compareTo(Month.MAY));
		System.out.println(month.compareTo(Month.APRIL));
		System.out.println(month.ordinal());

		Scanner sc = new Scanner(System.in);
		while (true) {
			System.out.println("Enter 1 to show moth by season");
			System.out.println("Enter 2 to show moth by count of days");
			switch (sc.next()) {
			case "1":
				System.out.println("Enter name of season");
				String nameOfSeason = sc.next();
				if (exist(Season.values(), nameOfSeason)) {
					Season season = Season.valueOf(nameOfSeason.toUpperCase());
					for (int i = 0; i < months.length; i++) {
